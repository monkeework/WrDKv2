<?php
function maxDoc_inc_themes(){
/**
 * common_inc.php stores site-wide utility functions
 *
 * Version 01 - initial prototyping/wiring/configuring
 *
 *
 * @package ma-v1605-22
...
$config->userLogin = ADMIN_PATH . 'login.php'; # Admin login page - all Admin pages part of nmAdmin package
 ...
 */
	...
	if(!defined('THEME_PHYSICAL')){define('THEME_PHYSICAL', PHYSICAL_PATH . '_themes/' . $config->theme . '/');}
	if(!defined('THEME_PATH')){define('THEME_PATH', VIRTUAL_PATH . '_themes/' . $config->theme . '/');}
	...
	{#part of benchmarking package - calculate on load of footer to be able to add page load to copyright for admins
		...
		if(startSession() && isset($_SESSION['UserID'])){ $config->copyright .= ' <em>(Page load: ' . $mytime . ' seconds.)</em> ';}
	}
 ...
 */
	...
	{# only admins can see edit fields
		echo '<div align="center"><form name="exitForm" action="' . $config->userDashboard . '" method="get">';
		...
		if (isset($_POST['submitForm']) && $_POST['submitForm'] == 1) { # If an administrator submits the form, update the page's RTEText.
			...
			{//only try to update if matches current RTEID
				...
				{# update!
					...
					$sql = sprintf("UPDATE RTE SET RTEText = '%s', Files = '%s', UserID=%d,LastUpdated=NOW() WHERE RTEID = %d",$RTEText,$myPages,$_SESSION['UserID'],$_POST['RTEID']);
				}else{# insert!
					$sql = sprintf("INSERT INTO RTE (RTEText,Files,UserID,LastUpdated) VALUES('%s','%s',%d,NOW())",$RTEText,THIS_PAGE,$_SESSION['UserID']);
				}
					$result = @mysqli_query(IDB::conn(),$sql) or die(trigger_error(mysqli_error(IDB::conn()), E_USER_ERROR));
				}
		}
 ...
 */
	...
	foreach($linkArray as $url => $text)
	{
		...
		if(
			!(strrpos($httpCheck,"http://")>-1) &&
			!(strrpos($httpCheck,"https://")>-1))
...
/* its an improved menu - has dropdowns built in */
function maxLinks($arrNewLinks, $prefix='', $suffix='', $separator="~"){
	/*
 * allows creation of links from associative array of link data and
 * HTML prefix & suffix to each link
 *
 ...
 */
	...
	if(!$adminOnly || startSession() && isset($_SESSION['UserID']))
...
function randomize ($arr){//randomize function is called in the left sidebar - an example of random (on page reload)
...
function pdo(){//return PDO object
/*
PDO & SQL Injection:
...
function safeEmail($to, $subject, $message, $replyTo='',$contentType='text/html; charset=ISO-8859-1 '){#builds and sends a safe email, using Reply-To properly!
