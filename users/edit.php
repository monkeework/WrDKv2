...
{//Select administrator
	...
	$iConn = IDB::conn();
	$sql = "select AdminID,UserName,Email,Privilege,LastLogin,NumLogins from " . PREFIX . "Admin";
	if($_SESSION["Privilege"] != "developer" && $_SESSION["Privilege"] != "superadmin")
...
function editDisplay()
{
	...
	$iConn = IDB::conn();
	$sql = sprintf("select UserName,Email,Privilege from " . PREFIX . "Admin WHERE AdminID=%d",$myID);
	$result = @mysqli_query($iConn,$sql) or die(trigger_error(mysqli_error($iConn), E_USER_ERROR));
	if(mysqli_num_rows($result) > 0)//at least one record!
	{//show results
		while ($row = mysqli_fetch_array($result))
...
function updateExecute()
{
	...
	$FirstName = iformReq('UserName',$iConn);  # iformReq calls dbIn() internally, to check form data
	...
	$sql = sprintf("UPDATE " . PREFIX . "Admin set FirstName='%s',Email='%s',Privilege='%s' WHERE AdminID=%d",$UserName,$Email,$Privilege,(int)$AdminID);
