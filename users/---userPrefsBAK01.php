...
<?php
	...
	{//check 'act' for type of process
		case "update": //2) Update my prefs
			updateExecute($uID); #Get from $_SESSION
			break;
...
function setStartPage($uID, $uStart, $str=''){//Select Customer
	...
	{//show results
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			echo '<tr>
				<td> &nbsp; <input type="radio" name="UserStartPage" value="userBegin"';
			...
			<tr>
				<td> &nbsp; <input type="radio" name="UserStartPage" value="dashboard"';
			...
			<tr>
				<td> &nbsp; <input type="radio" name="UserStartPage" value="reviewCharacters"';
	...
	if (mysqli_num_rows($result) > 0){//show results IF at least one record!
		...
		{//dbOut() function is a 'wrapper' designed to strip slashes, etc. of data leaving db
			...
			echo '<tr>
				<td> &nbsp; <input type="radio" name="UserStartPage" value="' . $cName . '"';
...
function updateExecute($uID){
	...
	Error in file: '/Applications/AMPPS/www/WrDKv2/users/userPrefs.php' on line: 366 Error message: You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near '='Wiccan' WHERE UserID=1' at line 1
