...
// END ERROR HANDLING - show/hide errors - turn on/off error logging
...
define('VIRTUAL_PATH', 'http://localhost/WrDKv2/'); # Virtual (web) 'root' of application for images, JS & CSS files
define('PHYSICAL_PATH', '/Applications/AMPPS/www/WrDKv2/'); # Physical (PHP) 'root' of application for file & upload reference
#define('INCLUDE_PATH', PHYSICAL_PATH . 'inc_0700/');
define('INCLUDE_PATH', PHYSICAL_PATH . '_inc/'); # SET to 0700
define('LOG_PATH', INCLUDE_PATH . 'log/'); # Log files are stored in the PHP include folder
define('ADMIN_PATH', VIRTUAL_PATH . 'users/'); # User/Admin files are in subfolder
define ('CURRENT_DIR', basename(dirname($_SERVER['PHP_SELF']))); #current folder i think?
define('SUPPORT_EMAIL', 'monkeework@gmail.com'); # Email of site support
define('PREFIX', 'ma_'); #Prefix to add uniqueness to your DB table names.  Limits hackability, naming collisions
...
define('SITE_NAME', 'Marvel Champions'); # SET to 0700
define('SITE_ABBR', 'MC'); #Abbreviated Name of site
...
# CONTENT CONFIG - theme, content, & nav arrays for header/footer
$config->theme = 'Bootswatch'; #default theme (header/footer combo) see 'Themes' folder for others and info
$config->style = 'yeti.css'; #currently only Bootswatch Theme uses style to switch look & feel
$config->slogan = 'Current Under Development (Neat huh?)';
$config->metaDescription = 'Welcome to Marvel Champions, an RPG based on the works of the Marvel Cinematic Universe!';
$config->metaKeywords = 'Roleplay, RPG, Marvel Champions, Super-heroes, Avengers, X-Men, Mutants, Fantastic Four, Wolverine, Captain America, Iron Man, Young Avengers, Wiccan, Hulkling, Spider-Man, S.H.I.E.L.D., SHIELD';
$config->metaRobots = 'no index, no follow';
$config->banner = 'Marvel Champions'; #goes inside header - can be overwritten
$config->copyright = 'monkeeworks, &copy; 2014 - ' . date('Y'); #goes inside footer - can be overwritten
...
if(startSession() && isset($_SESSION['UserID'])){#add admin logged in info to sidebar
	$config->sidebar2 .= '<p align="center">' . $_SESSION['Privilege'] . ' <b>' . $_SESSION['UserName'] . '</b> is logged in.</p>';
	$config->sidebar2 .= '<p align="center"><a href="' . $config->userDashboard . '">ADMIN</a></p>';
	$config->sidebar2 .= '<p align="center"><a href="' . $config->userLogout . '">LOGOUT</a></p>';
}
...
#GUEST FACING LINKS - IF NOT LOGGED IN/REGISTERED
if(startSession() && isset($_SESSION['UserID']) && $config->theme != 'Bootswatch' && $Privilege = 'developer'){$nav1[$config->userDashboard] = "ADMIN~Go to Administrative Page";}

#$nav1['index.php'] = "Home~A model for building largely static web pages";
$nav1['about.php'] = "About~Marvel Champions, a cohesive re-imagining of the Marvel Cinematic Universe";
$nav1['characters/index.php'] = "Characters~Character Creation Start Page";



#CONFIG contact/eMail/mail/notifications HERE
$toAddress = "speedlanerunner@yahoo.com, monkeework@gmail.com, chezshire@gmail.com";  //place your/your client's email address here ADDRESSES!
$toName = "Grandmaster, Gardener , Info"; //place your client's name here
...
if(startSession() && isset($_SESSION['UserID'])){
	...
	$nav1['users/dashboard.php'] = "Dashboard~Link to user/admin dashboard";
	$nav1['_test/'] = "Test~Build Block Files Go Here";
	$nav1['_test/shared.php'] = "MySQLi Shared~A demo page for building mysqli shared connection based applications.";
	$nav1['_test/pdo.php'] = "PDO~A demo page for building PDO connection based applications.";
	$nav1['_test/contact.php'] = "Contact~A demo for building postback forms";
}
...
function myErrorHandler ($e_number, $e_message, $e_file, $e_line, $e_vars){
	...
	if (SHOW_ALL_ERRORS || isset($_SESSION['UserID']))
