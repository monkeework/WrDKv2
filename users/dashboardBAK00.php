...
//BEGIN character review que
				...
				if(mysqli_num_rows($result) > 0){//records exist - process
					...
					{// process each row
						...
						//get image
						//if image set show, else show proxy or default)
						if (file_exists($cImg)) { //if handler did, show us image
							echo '<img src="' . $cImg . '" class="center-block img-responsive img-circle img-review" alt="' . $cName . '" style="height:150px; ' . $statusBorder . '"/></a>';

						} elseif(file_exists($cbLnk)) { //if we can match playby, map to playby as proxy
							//if playby, chek formatch?
							echo '<img src="' . $cbLnk . '" class="center-block img-responsive img-circle img-review" style="width:150px; hieght:150px; ' . $statusBorder . ' " alt="' . $cName . '" />';

						//no match, no proxy
						} else { //show default
							echo '<img src="' . $sImg . '" class="center-block img-responsive img-circle img-review" style="width:150px; ' . $statusBorder . '" alt="' . $cName . '" />';
						}

						//close up link
						echo '</a>
							<h5><a href="' . $cLink . '" title="' . $cName . '" target="_blank">' . $cName . '</a></h5>
							<span class="text-muted">
							' . $aarStatus[$cStatus] . ' | <a href="
							' . $cEdit . '" target="_blank" title="'
							. $cName . '">Edit</a> | HandlerName</span>
							<br /><br /></div>'; //close character
					}
