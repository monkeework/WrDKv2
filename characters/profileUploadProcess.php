...
if(isset($_POST['type'])){$type 			= $_POST['type'];}else{$thumbWidth = "";}
#a-zz so we can have
...
if($type == 'p'){
	#if profile image
	$fileName   = $myID . '-' . $series . $extension;  //create name of file to be uploaded
	$thumbName  = $myID . '-' . $series . 't' . $extension;  //create name of file to be uploaded
} else if($type == 'g'){
	#if gallery image
	$fileName   = $myID . '-' . $series . 'g' . $extension;  //create name of file to be uploaded
	$thumbName  = $myID . '-' . $series . 't' . $extension;  //create name of file to be uploaded
}else{
	#if banner header image
	$fileName   = $myID . '-h' . $series . $extension;  //create name of file to be uploaded
	$thumbName  = $myID . '-h' . $series . 't' . $extension;  //create name of file to be uploaded
	$sizeBytes  = "800000";
}
...
string '15-2.jpg' (length=8)

string '15-2t.jpg' (length=9)
var_dump($fileName);
echo '<br />';
dumpDie($thumbName);
